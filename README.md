...
- Secure hosted-image insertion from URL or local upload through `api/documents`, with Cloudinary storage, preview, accessibility metadata, and base64 image storage blocked
...
Cloudinary variables belong in `api/documents`, not this frontend app. Do not expose them with `NEXT_PUBLIC_`.
